...
the LangGraph workflow exactly once and registers all API routers — without
...
def test_create_app_builds_graph_once_and_registers_routers() -> None:
    """Spec: startup-singleton / server-starts — graph built once, all routes registered."""
    with (
    ...
    assert str(app.url_path_for("execute_query")) == "/api/query"
