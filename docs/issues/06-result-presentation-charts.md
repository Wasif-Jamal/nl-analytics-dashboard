...
> Implemented by the Visualization Agent subagent — a `create_agent()` instance with its own LLM and internal tools. Reads `query_result` from `WorkflowState`; the agent's LLM selects the chart type and generates the config. Depends on issue #4 (agent pattern established).
