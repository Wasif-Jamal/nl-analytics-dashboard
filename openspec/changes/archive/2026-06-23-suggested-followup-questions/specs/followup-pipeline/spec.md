# Spec Delta: followup-pipeline (suggested-followup-questions change)

## Delta Summary

Introduces the FollowupAgent (FR-10): replaces the pass-through stub with a
`create_agent()` instance, adds `FollowupTools` with a `generate_followup_questions`
tool, and adds the `FollowupOutput` Pydantic schema. All requirements in this file are
new additions — this is a new spec domain.
